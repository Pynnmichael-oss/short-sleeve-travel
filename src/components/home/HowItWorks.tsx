  ...
  return (
    <section className="bg-forest py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="font-display text-4xl md:text-5xl text-offwhite mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16">
          {steps.map(({ number, title, description }) => (
            <div key={number} className="flex flex-col gap-4">
              <span className="font-display text-5xl text-sand/40 leading-none">
                {number}
              </span>
              <h3 className="font-display text-2xl text-offwhite">{title}</h3>
              <p className="font-body text-warmwhite/60 leading-relaxed">
                {description}
              </p>
            </div>
          ))}
