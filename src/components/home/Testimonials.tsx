  ...
  return (
    <section className="bg-offwhite py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        <SectionLabel className="text-sand mb-12 block">
          From the community
        </SectionLabel>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16">
          {testimonials.map(({ quote, name, detail }) => (
            <blockquote key={name} className="flex flex-col gap-6">
              <span className="font-display text-6xl text-sand/30 leading-none select-none">
                "
              </span>
              <p className="font-display text-2xl md:text-3xl text-charcoal italic leading-snug -mt-4">
                {quote}
              </p>
              <footer className="font-body text-sm text-charcoal/50">
                — {name}, {detail}
              </footer>
            </blockquote>
          ))}
